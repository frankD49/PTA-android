...
do $$ begin
  if not exists(select 1 from vault.secrets where name='project_url') then perform vault.create_secret('https://pvhfkjinyrgxakvsoblp.supabase.co','project_url','Supabase project URL for scheduled functions'); end if;
  if not exists(select 1 from vault.secrets where name='compliance_cron_secret') then perform vault.create_secret(replace(gen_random_uuid()::text,'-','')||replace(gen_random_uuid()::text,'-',''),'compliance_cron_secret','Authentication token for compliance cron'); end if;
end $$;
create or replace function public.verify_compliance_cron_secret(candidate text)
returns boolean language sql stable security definer set search_path=vault as $$ select candidate is not null and exists(select 1 from vault.decrypted_secrets where name='compliance_cron_secret' and decrypted_secret=candidate); $$;
...
begin
  ...
  with due as (
    ...
    where status in ('pending','processing') and scheduled_for<=now()
      and coalesce(next_attempt_at,scheduled_for)<=now() and attempt_count<10
    order by coalesce(next_attempt_at,scheduled_for) for update skip locked
    limit greatest(1,least(batch_size,100))
  ), claimed as (
    update public.account_deletion_requests r set status='processing',attempt_count=attempt_count+1,last_attempt_at=now(),last_error=null
...
begin
  update public.account_deletion_requests set last_error=left(failure,2000),
    processing_step=case when attempt_count>=10 then 'failed' else processing_step end,
...
begin
  ...
  if project_url is null or cron_secret is null then
    raise warning 'Deletion cron not dispatched: Vault secrets project_url/compliance_cron_secret are required'; return;
  end if;
  perform net.http_post(url:=rtrim(project_url,'/')||'/functions/v1/compliance-operations',
    headers:=jsonb_build_object('Content-Type','application/json','x-cron-secret',cron_secret),
    body:='{"action":"processDueDeletions","batchSize":25}'::jsonb,timeout_milliseconds:=30000);
end;
$$;
revoke all on function public.dispatch_due_account_deletions() from public,anon,authenticated;
do $$ begin
  if exists(select 1 from cron.job where jobname='process-due-account-deletions') then perform cron.unschedule('process-due-account-deletions'); end if;
